    ...
    <div className='Customer px-[20px]'>
       ...
       <div className="content">
        <div className="filter border-[#d8d8d8f2] border-b-[3px] py-[15px]"> 
          <div className="flex justify-center items-center gap-[10%] py-[10px] h-[50px]">  
            <form className='flex gap-[10px] items-center ' onSubmit={handleSubmit(onSubmit)}>
              <div name='phone' className='mb-0 relative' > 
                <input value={dataSearch} onKeyDown={(e)=>{
                  if(e.which===13){
                    handleChangeInputPhoneNumber();
                  }
                }} onChange={(e) => {setDataSearch(e.target.value);console.log(dataSearch); handlePhoneChange()}} className='outline-0 px-[30px] py-[10px] rounded-[5px] shadow-md  shadow-[rgba(0, 0, 0, 0.15) 0px 5px 15px 0px]'  
